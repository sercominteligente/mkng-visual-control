...
export function FinancePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(finance.items); setCustomers(clientData.items); setSuppliers(supplierData.items);
  ...
  const save = async (event: React.FormEvent) => { event.preventDefault(); await api("/finance", { method: "POST", body: JSON.stringify({ ...form, kind }) }); setOpen(false); await load(); };
  const markPaid = async (item: any) => { await api(`/finance/${kind}/${item.id}`, { method: "PUT", body: JSON.stringify({ ...item, status: "paid" }) }); await load(); };
