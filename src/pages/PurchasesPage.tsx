...
export function PurchasesPage({ user }: { user: User }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(purchases.items); setSuppliers(supplierData.items); setMaterials(materialData.items); setError("");
    } catch (err) { setError(err instanceof Error ? err.message : "Falha ao carregar compras"); }
  ...
  const receive = async (purchase: any) => {
    ...
    try { await api(`/purchases/${purchase.id}/receive`, { method: "POST" }); await load(); }
    catch (err) { setError(err instanceof Error ? err.message : "Não foi possível receber a compra"); }
  };
  const remove = async (purchase: any) => {
    const confirmation = window.prompt(`Excluir definitivamente a compra ${purchase.code}?\n\nEntradas de estoque e contas a pagar vinculadas serão revertidas/removidas. Digite EXCLUIR para confirmar.`);
    if (confirmation !== "EXCLUIR") return;
    try { await api(`/purchases/${purchase.id}`, { method: "DELETE" }); await load(); }
