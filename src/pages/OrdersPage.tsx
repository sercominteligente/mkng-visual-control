...
const lossLabels: Record<string, string> = { operational: "Operacional", setup: "Configuração", human_error: "Erro humano", material_defect: "Defeito do material", scrap: "Sobra não reaproveitável" };
...
function OrderDetail({ detail, materials, onClose, onCancel, onDelete, onChanged }: { detail: any; materials: any[]; onClose: () => void; onCancel: () => void; onDelete: () => void; onChanged: () => Promise<void> }) {
  ...
  const pendingMaterials = (detail.materials || []).filter((item: any) => Math.max(Number(item.reserved_qty || 0) - Number(item.consumed_qty || 0), 0) > 0.0001);
  ...
  const confirmConsume = async () => {
    try {
      setError(""); setNotice("");
      const result = await api<{ newStock: number; quantity: number; unit: string }>(`/orders/${detail.order.id}/consume`, { method: "POST", body: JSON.stringify(consume) });
      setNotice(`Baixa confirmada: ${result.quantity} ${result.unit}. Novo saldo: ${result.newStock} ${result.unit}.`);
      await onChanged();
    } catch (err) { setNotice(""); setError(err instanceof Error ? err.message : "Falha ao confirmar consumo"); }
  };

  const changeStage = async () => {
    try {
      setError(""); setNotice("");
      const autoConsumePending = stage === "completed" && pendingMaterials.length > 0;
      if (autoConsumePending) {
        const summary = pendingMaterials.map((item: any) => `${item.material_name}: ${Math.max(Number(item.reserved_qty || 0) - Number(item.consumed_qty || 0), 0).toLocaleString("pt-BR", { maximumFractionDigits: 3 })} ${item.unit}`).join("\n");
        const confirmed = window.confirm(`Concluir o pedido e dar baixa automática nos materiais pendentes?\n\n${summary}`);
        if (!confirmed) return;
      }
      const result = await api<{ autoConsumed?: any[] }>(`/orders/${detail.order.id}/stage`, { method: "POST", body: JSON.stringify({ stage, auto_consume_pending: autoConsumePending }) });
      setNotice(stage === "completed"
        ? result.autoConsumed?.length
          ? `Pedido concluído e ${result.autoConsumed.length} material(is) baixado(s) automaticamente.`
          : "Pedido concluído com todas as baixas confirmadas."
        : "Etapa atualizada com sucesso.");
      await onChanged();
    } catch (err) { setNotice(""); setError(err instanceof Error ? err.message : "Falha ao avançar etapa"); }
  };
  ...
  return <>
    <Modal title={`${detail.order.code} — ${detail.order.title}`} onClose={onClose} width={1100}>
      <div className="detail-toolbar">
        <button className="primary-button" onClick={printCurrent}>Imprimir Pedido / OS</button>
        {!locked && <button className="secondary-button" onClick={() => setLossOpen(true)}>Registrar perda</button>}
        {detail.permissions?.canCancel && <button className="secondary-button action-danger" onClick={onCancel}>Cancelar pedido</button>}
        {detail.permissions?.canDeletePermanent && <button className="danger-button" onClick={onDelete}>Excluir definitivamente</button>}
      </div>
      {error && <div className="alert error">{error}</div>}
      {notice && <div className="alert success">{notice}</div>}
      <div className="detail-grid">
        <div className="detail-card"><span>Cliente</span><strong>{detail.order.customer_name || "—"}</strong></div>
        <div className="detail-card"><span>Status</span><Badge tone={tones[detail.order.status]}>{labels[detail.order.status]}</Badge></div>
        <div className="detail-card"><span>Entrega</span><strong>{dateBR(detail.order.due_date)}</strong></div>
        {detail.permissions?.canViewFinancial && <div className="detail-card"><span>Valor</span><strong>{brl(detail.order.total_price)}</strong></div>}
      </div>
      <div className="two-columns">
        <section className="panel inner-panel">
          <div className="panel-head"><h3>Materiais do pedido</h3></div>
          {detail.materials.length === 0 ? <p className="muted">Nenhum material planejado.</p> : <div className="table-wrap"><table><thead><tr><th>Material</th><th>Planejado</th><th>Reservado</th><th>Consumido</th><th>Perda</th><th>Reimpressão</th><th>Pendente</th><th>Devolvido</th></tr></thead><tbody>{detail.materials.map((item: any) => { const pending = Math.max(Number(item.reserved_qty || 0) - Number(item.consumed_qty || 0), 0); return <tr key={item.id}><td>{item.material_name}</td><td>{item.planned_qty} {item.unit}</td><td>{item.reserved_qty} {item.unit}</td><td>{item.consumed_qty} {item.unit}</td><td className={Number(item.loss_qty || 0) > 0 ? "negative-value" : ""}>{item.loss_qty || 0} {item.unit}</td><td>{item.reprint_qty || 0} {item.unit}</td><td>{pending.toLocaleString("pt-BR", { maximumFractionDigits: 3 })} {item.unit}</td><td>{item.returned_qty} {item.unit}</td></tr>; })}</tbody></table></div>}
        </section>
        <section className="panel inner-panel">
          <div className="panel-head"><div><h3>Confirmar consumo</h3><p>A baixa respeita a unidade de estoque cadastrada: unidade, chapa, rolo, litro, metro ou m².</p></div></div>
          {locked ? <p className="muted">Consumo bloqueado para pedido {labels[detail.order.status].toLowerCase()}.</p> : <div className="stack-form">
            <label><span>Material</span><select value={consume.material_id} onChange={(event) => setConsume({ ...consume, material_id: event.target.value, mode: "quantity" })}>{materials.filter((material) => material.active !== 0).map((material) => <option key={material.id} value={material.id}>{material.name} — saldo {Number(material.current_stock).toLocaleString("pt-BR", { maximumFractionDigits: 3 })} {material.unit}</option>)}</select></label>
            {selectedMaterial?.unit === "m2" && <div className="consumption-mode"><button type="button" className={consume.mode === "quantity" ? "active" : ""} onClick={() => setConsume({ ...consume, mode: "quantity" })}>Informar m²</button><button type="button" className={consume.mode === "dimensions" ? "active" : ""} onClick={() => setConsume({ ...consume, mode: "dimensions" })}>Calcular por medidas</button></div>}
            {selectedMaterial?.unit === "m2" && consume.mode === "dimensions" ? <div className="area-calculator"><label><span>Largura (mm)</span><input type="number" min="1" step="1" value={consume.width_mm} onChange={(event) => setConsume({ ...consume, width_mm: Number(event.target.value) })} /></label><label><span>Altura (mm)</span><input type="number" min="1" step="1" value={consume.height_mm} onChange={(event) => setConsume({ ...consume, height_mm: Number(event.target.value) })} /></label><label><span>Quantidade de peças</span><input type="number" min="1" step="1" value={consume.pieces} onChange={(event) => setConsume({ ...consume, pieces: Number(event.target.value) })} /></label><div className="calculated-area"><span>Área calculada</span><strong>{calculatedArea.toLocaleString("pt-BR", { maximumFractionDigits: 3 })} m²</strong></div></div> : <label><span>Quantidade consumida ({selectedMaterial?.unit || "un"})</span><input type="number" min={( ["un", "chapa", "rolo", "lata", "kit", "pct"].includes(selectedMaterial?.unit) ? 1 : 0.001)} step={( ["un", "chapa", "rolo", "lata", "kit", "pct"].includes(selectedMaterial?.unit) ? 1 : 0.001)} value={consume.quantity} onChange={(event) => setConsume({ ...consume, quantity: Number(event.target.value) })} /></label>}
            <input placeholder="Observação" value={consume.notes} onChange={(event) => setConsume({ ...consume, notes: event.target.value })} />
            <button className="primary-button" disabled={selectedMaterial?.unit === "m2" && consume.mode === "dimensions" && calculatedArea <= 0} onClick={() => void confirmConsume()}>Confirmar baixa</button>
          </div>}
        </section>
      </div>
      <section className="panel inner-panel">
        <div className="panel-head"><div><h3>Perdas e reimpressões</h3><p>O material perdido baixa o estoque e o custo permanece vinculado ao Job.</p></div>{!locked && <button className="secondary-button" onClick={() => setLossOpen(true)}>+ Registrar perda</button>}</div>
        {!detail.losses?.length ? <p className="muted">Nenhuma perda registrada neste pedido.</p> : <div className="table-wrap"><table><thead><tr><th>Data</th><th>Material</th><th>Tipo</th><th>Quantidade</th><th>Motivo</th><th>Máquina</th><th>Operador</th><th>Custo</th><th>Reimpressão</th><th>Status</th></tr></thead><tbody>{detail.losses.map((loss: any) => <tr key={loss.id} className={loss.status === "reversed" ? "row-inactive" : ""}><td>{dateTimeBR(loss.created_at)}</td><td>{loss.material_name}</td><td>{lossLabels[loss.loss_type] || loss.loss_type}</td><td className="negative-value">{loss.quantity} {loss.unit}</td><td>{loss.reason}</td><td>{loss.machine || "—"}</td><td>{loss.created_by_name || "Sistema"}</td><td>{brl(loss.total_cost || 0)}</td><td>{loss.requires_reprint ? `${loss.reprint_qty} ${loss.unit}` : "Não"}</td><td><Badge tone={loss.status === "confirmed" ? "danger" : "neutral"}>{loss.status === "confirmed" ? "Confirmada" : "Estornada"}</Badge></td></tr>)}</tbody></table></div>}
      </section>
      <section className="panel inner-panel">
        <div className="panel-head"><div><h3>Produção</h3><p>Avance o pedido pela linha de produção.</p></div>{!locked && <div className="inline-actions"><select value={stage} onChange={(event) => setStage(event.target.value)}><option value="briefing">Briefing</option><option value="design">Criação / arte</option><option value="printing">Impressão</option><option value="finishing">Acabamento</option><option value="installation">Instalação</option><option value="completed">Concluído</option></select><button className="secondary-button" title={stage === "completed" && pendingMaterials.length > 0 ? "Ao concluir, o sistema dará baixa automática nos materiais reservados ainda pendentes" : undefined} onClick={() => void changeStage()}>{stage === "completed" && pendingMaterials.length > 0 ? "Concluir e baixar materiais" : "Avançar etapa"}</button></div>}</div>
        {!locked && pendingMaterials.length > 0 && <div className="alert warning">Existem materiais reservados ainda sem baixa. Você pode confirmar o consumo manualmente ou concluir o pedido para aplicar a baixa automática.</div>}
        <div className="timeline">{detail.steps.map((step: any) => <div key={step.id} className={`timeline-item ${step.status}`}><i /><div><strong>{step.stage}</strong><small>{step.status} · {step.assignee_name || "Sem responsável"}</small></div></div>)}</div>
      </section>
      <section className="panel inner-panel">
        <div className="panel-head"><div><h3>Histórico e movimentações</h3><p>Cada evento mantém um snapshot auditável e pode gerar seu próprio PDF.</p></div></div>
        {!detail.events?.length ? <p className="muted">Nenhuma movimentação registrada.</p> : <div className="event-list">{detail.events.map((event: any) => <article className="event-row" key={event.id}><div className="event-dot" /><div className="event-content"><strong>{event.label}</strong><small>{dateTimeBR(event.created_at)} · {event.user_name || "Sistema"} · {labels[event.status] || event.status || "—"}</small>{event.notes && <p>{event.notes}</p>}</div><button className="secondary-button" onClick={() => printEvent(event.id)}>Imprimir PDF</button></article>)}</div>}
      </section>
    </Modal>
    {lossOpen && <Modal title={`Registrar perda — ${detail.order.code}`} onClose={() => setLossOpen(false)} width={980}><LossForm materials={materials} fixedOrderId={detail.order.id} fixedOrderCode={detail.order.code} onCancel={() => setLossOpen(false)} onSaved={async (result) => { setLossOpen(false); setNotice(`Perda registrada: ${result.quantity} ${result.unit}. Novo saldo: ${result.newStock} ${result.unit}.`); await onChanged(); }} /></Modal>}
  </>;
}
