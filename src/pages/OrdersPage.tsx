...
export function OrdersPage({ user }: { user: User }) {
  ...
  const confirmDelete = async () => {
    if (!deleteOrder || deleteConfirmation !== "EXCLUIR RASCUNHO") return;
    try {
      await api(`/orders/${deleteOrder.id}`, { method: "DELETE" });
      setDeleteOrder(null); setDeleteConfirmation(""); setDetail(null); await load();
    } catch (err) { setError(err instanceof Error ? err.message : "Não foi possível excluir o rascunho"); }
  };
  ...
  return <>
    ...
    {deleteOrder && <Modal title={`Excluir rascunho ${deleteOrder.code}`} onClose={() => setDeleteOrder(null)}><div className="stack-form"><div className="alert error">A exclusão é definitiva e exclusiva do Super Administrador. Itens preliminares, reservas e anexos do rascunho também serão removidos.</div><label><span>Digite EXCLUIR RASCUNHO para confirmar</span><input autoFocus value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} /></label><div className="form-actions"><button className="ghost-button" onClick={() => setDeleteOrder(null)}>Cancelar</button><button className="danger-button" disabled={deleteConfirmation !== "EXCLUIR RASCUNHO"} onClick={() => void confirmDelete()}>Excluir definitivamente</button></div></div></Modal>}
